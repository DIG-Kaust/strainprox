...
def callbackx(x, ui, xtrue, xhist, xsnr, xerr):
    x += ui 
    xhist.append(x)
...
def callbackxy(x, y, xtrue, xhist, yhist, xsnr, ysnr, xerr, yerr):
    xhist.append(x)
    yhist.append(x)
    xsnr.append(SNR(xtrue, x))
...
def plotter_4D(b, m, dt=1, type='impedance', perc=1, dif_scale=0.01, ref=None,
               height=4, width=15, mtrue=None, cmap='seismic_r', vline=None, ztitle=''):
...
def plotter_timeshift(d1, d2, d2s, shift, dt=1, perc=1, height=5, width=20, dif_scale=.03, alpha=0.7, cmap='RdGy'):
...
def plotter_timestrain(b, m, strain, mshift, dt=1, perc=1, height=5, width=15, dif_scale=1., alpha=0.7, years=[1994, 2001]):
    ...
    ts = ax1.imshow(strain*100, vmin=-1*dif_scale, vmax=1*dif_scale, cmap='PiYG', alpha=alpha)
    ax1.set_yticklabels([])
